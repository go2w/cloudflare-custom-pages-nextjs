...
export const CFCard = ({
  ...
  const finalIconClassName = iconClassName || cx("bg-gradient-to-br", schemeClasses.gradient, schemeClasses.iconBg);
  ...
  return (
    <Card
      ...
      <CardHeader className={cx("relative p-4 sm:p-6 rounded-t-xl", headerClassName)}>
        {watermark && <div className='absolute right-0 top-0 h-16 sm:h-24 w-16 sm:w-24 opacity-20'>{watermark}</div>}
        ...
        <div className='flex flex-row items-center gap-3 sm:gap-4'>
          <div className='flex-shrink-0'>
            <div className='relative w-10 h-10 sm:w-12 sm:h-12'>
              <div className={cx("absolute inset-0 animate-pulse rounded-full blur-xl", getIconGlowClass())} />
              <div
                className={cx(
                  "relative rounded-full p-2 sm:p-2.5 flex items-center justify-center h-full",
                  finalIconClassName
                )}>
                <div className='w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center'>{icon}</div>
              </div>
            </div>
          </div>
          ...
          <div className='flex flex-col min-w-0'>
            {subtitle &&
              (typeof subtitle === "string" ?
                <span className='text-sm font-medium text-default-600 dark:text-default-400 truncate'>{subtitle}</span>
              : subtitle)}
