  ...
  return (
    <Link
      ...
      <div className='flex items-center gap-2 sm:gap-2.5 md:gap-3'>
        ...
        <span className='text-sm sm:text-base text-gray-800 dark:text-gray-200 font-medium'>{page.title}</span>
      </div>
      <div className='flex items-center gap-1.5 sm:gap-2'>
        {page.code && (
          <span
            className={cx("text-xs px-1.5 sm:px-2 py-0.5 font-mono rounded-full", classes.codeBg, classes.codeText)}>
            {page.code}
          </span>
        )}
        <Icon
