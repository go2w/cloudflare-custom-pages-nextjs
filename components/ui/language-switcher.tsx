...
export function LanguageSwitcher() {
  ...
  const getCurrentLanguageInfo = () => {
    ...
    return languages.find(lang => lang.code === currentLanguage) || languages[1]; // languages[1] 是 'en'
  };
        ...
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 min-w-[160px] bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2">
            {languages.map((language) => {
              const isSelected = language.code === 'auto' ? isAutoMode : language.code === currentLanguage && !isAutoMode;
                  ...
                  <div className="flex flex-col">
                    ...
                    {language.code === 'auto' && (
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        检测到: {languages.find(l => l.code === detectedLanguage)?.name || 'English'}
                      </span>
                    )}
                  </div>
                  {isSelected && (
